...
declare namespace ts.pxtc {
    namespace hexfile {
        ...
        function setupFor(opts: CompileTarget, extInfo: ExtensionInfo): void;
...
declare namespace ts.pxtc.service {
    ...
    type OpRes = string | void | SyntaxInfo | CompletionInfo | CompileResult | transpile.TranspileResult | {
        words: number[];
    } | KsDiagnostic[] | {
        formatted: string;
        pos: number;
    ...
    type OpResOrError = OpRes | OpError;
    function IsOpErr(res: OpResOrError): res is OpError;
    function performOperation<T extends keyof ServiceOps>(op: T, arg: OpArg): OpResOrError;
    function getSnippet(apis: ApisInfo, takenNames: pxt.Map<SymbolInfo>, runtimeOps: pxt.RuntimeOptions, fn: SymbolInfo, decl: ts.FunctionLikeDeclaration, python?: boolean, recursionDepth?: number): string;
}
