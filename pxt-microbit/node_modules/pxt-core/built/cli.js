...
pxt.docs.requireDOMSanitizer = function () { return require("sanitize-html"); };
...
function parseBuildInfo(parsed) {
    ...
    var hwvariant = parseHwVariant(parsed);
...
function checkIfTaggedCommitAsync() {
    ...
    return nodeutil.gitInfoAsync(["rev-parse", "HEAD"])
        ...
        .then(function (info) {
        var tagCommits = info.split("\n")
            .map(function (line) {
            var match = /^([a-fA-F0-9]+)\s+refs\/tags\/v\d+\.\d+\.\d+\^\{\}$/.exec(line);
            return match && match[1];
        });
        return tagCommits.some(function (t) { return t === currentCommit; });
    });
}
var readJson = nodeutil.readJson;
function ciAsync() {
    forceCloudBuild = true;
    var buildInfo = ciBuildInfo();
    pxt.log("ci build using " + buildInfo.ci);
    if (!buildInfo.tag)
        buildInfo.tag = "";
    if (!buildInfo.branch)
        buildInfo.branch = "local";
    var tag = buildInfo.tag, branch = buildInfo.branch, pullRequest = buildInfo.pullRequest;
    var atok = process.env.NPM_ACCESS_TOKEN;
    var npmPublish = /^v\d+\.\d+\.\d+$/.exec(tag) && atok;
    if (npmPublish) {
    ...
    var masterOrReleaseBranchRx = /^(master|v\d+\.\d+\.\d+)$/;
    var apiStringBranchRx = pxt.appTarget.uploadApiStringsBranchRx
        ? new RegExp(pxt.appTarget.uploadApiStringsBranchRx)
        : masterOrReleaseBranchRx;
    var uploadDocs = !pullRequest
        && !!pxt.appTarget.uploadDocs
        && masterOrReleaseBranchRx.test(branch);
    var uploadApiStrings = !pullRequest
    ...
    if (pkg["name"] == "pxt-core") {
        ...
        return checkIfTaggedCommitAsync()
            .then(function (isTaggedCommit) {
            pxt.log("is tagged commit: " + isTaggedCommit);
            var p = npmPublishAsync();
            if (branch === "master" && isTaggedCommit) {
                ...
                if (uploadApiStrings)
                    p = p.then(function () { return crowdin.execCrowdinAsync("upload", "built/strings.json"); });
                if (uploadDocs || uploadApiStrings)
                    p = p.then(function () { return crowdin.internalUploadTargetTranslationsAsync(uploadApiStrings, uploadDocs); });
            }
            return p;
        });
    }
...
function uploadCoreAsync(opts) {
    ...
    if (opts.localDir) {
        ...
        var targetImagePaths = targetImages.map(function (k) {
    ...
    function encodeURLs(urls) {
        return urls.map(function (url) { return encodeURIComponent(url); }).join(";");
    }
    var uplReqs = {};
...
function internalBuildTargetAsync(options) {
    ...
    return initPromise
        ...
        .then(function () { return buildFolderAsync('compiler', true, 'compiler'); })
        .then(function () { return fillInCompilerExtension(pxt.appTarget); })
...
function buildEditorExtensionAsync(dirname, optionName) {
    if (pxt.appTarget.appTheme && pxt.appTarget.appTheme[optionName] &&
        ...
        if (tsConfig.compilerOptions.module)
            p_1 = buildFolderAndBrowserifyAsync(dirname, true, dirname);
        else
            p_1 = buildFolderAsync(dirname, true, dirname);
        return p_1.then(function () {
...
function ciBuildInfo() {
    ...
    function githubActionInfo() {
        // https://help.github.com/en/actions/automating-your-workflow-with-github-actions/using-environment-variables#default-environment-variables
        var repoSlug = process.env.GITHUB_REPOSITORY;
        var commit = process.env.GITHUB_SHA;
        var ref = process.env.GITHUB_REF;
        // branch build refs/heads/...
        // tag build res/tags/...
        var branch = ref.replace(/^refs\/(heads|tags)\//, '');
        var tag = /^refs\/tags\//.test(ref) ? branch : undefined;
...
function saveThemeJson(cfg, localDir, packaged) {
    ...
    if ((_c = (_b = (_a = cfg.runtime) === null || _a === void 0 ? void 0 : _a.functionsOptions) === null || _b === void 0 ? void 0 : _b.extraFunctionEditorTypes) === null || _c === void 0 ? void 0 : _c.length) {
        cfg.runtime.functionsOptions.extraFunctionEditorTypes.forEach(function (extraType) {
            if (extraType.label) {
    ...
    if (nodeutil.fileExistsSync("targetconfig.json")) {
        ...
        if (targetConfig_1 && targetConfig_1.galleries) {
            ...
            Object.keys(targetConfig_1.galleries).forEach(function (k) {
                targetStrings[k] = k;
                var galleryUrl = getGalleryUrl(targetConfig_1.galleries[k]);
                var gallerymd = nodeutil.resolveMd(docsRoot_1, galleryUrl);
                var gallery = pxt.gallery.parseGalleryMardown(gallerymd);
                var gurl = "/" + galleryUrl.replace(/^\//, '');
                tocmd_1 +=
                    "* [" + k + "](" + gurl + ")\n";
                var gcard = {
                    name: k,
    ...
    Object.keys(targetStrings).sort().map(function (k) { return targetStringsSorted[k] = targetStrings[k]; });
    // write files
...
function compressApiInfo(inf) {
    function leanSymbol(sym) {
        ...
        var pyQName = sym.pyQName !== sym.qName ? sym.pyQName : undefined;
        return {
    ...
    return buildWebStringsAsync()
        ...
        return pkg.filesToBePublishedAsync(true)
            ...
            if (isPrj && pxt.appTarget.compile && pxt.appTarget.compile.hasHex) {
                ...
                var hex = options.extinfo.hexinfo.hex;
        ...
        if (coreInfo) {
            ...
            if (coreDependencies) {
                coreDependencies
                    .map(function (dep) { return builtInfo["libs/" + dep]; })
                    .filter(function (bi) { return !!bi; })
                    .forEach(function (bi) { return bi.apis.byQName = {}; });
            }
...
function fillInCompilerExtension(cfg) {
    var compPath = path.join(nodeutil.targetDir, "built/compiler.js");
    if (fs.existsSync(compPath)) {
        var src = fs.readFileSync(compPath, "utf8");
...
function buildAndWatchTargetAsync(includeSourceMaps, rebundle) {
    ...
    return buildAndWatchAsync(function () { return buildCommonSimAsync()
        .catch(function (e) { return buildFailed("common sim build failed: " + e.message, e); })
...
function downloadPlaylistsAsync(parsed) {
    var fn = (parsed.args[0] || "playlists.json");
    if (!nodeutil.fileExistsSync(fn))
        U.userError("File " + fn + " not found");
    var playlists = nodeutil.readJson(fn);
...
function downloadDiscourseTagAsync(parsed) {
    ...
    var out = parsed.flags["out"] || "temp";
    var outmd = parsed.flags["md"];
    var discourseRoot = pxt.appTarget.appTheme
    ...
    var md = outmd && fs.readFileSync(outmd, { encoding: "utf8" });
    nodeutil.mkdirP(out);
    var n = 0;
    var newcards = 0;
    var cards = [];
    ...
    return pxt.discourse.topicsByTag(discourseRoot, tag)
        .then(function (topics) { return Promise.mapSeries(topics, function (topic) {
        pxt.log("  " + topic.title);
        return pxt.discourse.extractSharedIdFromPostUrl(topic.url)
            ...
            return extractAsyncInternal(id, out, false)
                ...
                var card = cards.filter(function (c) { return c.url == topic.url; })[0];
                ...
                card.description = "";
                cards.push(card);
                var pfn = "./docs/static/discourse/" + id + ".";
                if (md && !["png", "jpg", "gif"].some(function (ext) { return nodeutil.fileExistsSync(pfn + ext); })) {
        ...
        if (md) {
            ...
            if (lastCard)
                cards.push(lastCard);
            cards.forEach(function (card) { return delete card.id; });
            md = md.replace(rx, function (m, c) {
    ...
    function downloadImageAsync(id, topic, url) {
        return pxt.Util.requestAsync({
            ...
            if (resp.buffer) {
                var m = /image\/(png|jpeg|gif)/.exec(resp.headers["content-type"]);
                if (!m) {
                    ...
                    if (ext == "jpeg")
                        ext = "jpg";
                    var ifn = "/static/discourse/" + id + "." + ext;
                    var localifn = "./docs" + ifn;
...
function testSnippetsAsync(snippets, re, pyStrictSyntaxCheck) {
    ...
    var addFailure = function (f, infos) {
        ...
        infos.forEach(function (info) { return pxt.log(f + ":(" + info.line + "," + info.column + "): " + info.category + " " + info.messageText); });
    };
    return Promise.map(snippets, function (snippet) {
        ...
        return pkg.installAllAsync()
            ...
            if (isPy) {
                ...
                var stsCompRes = pxtc.compile(opts);
                var apisInfo = pxtc.getApiInfo(stsCompRes.ast, opts.jres);
                if (!apisInfo || !apisInfo.byQName)
                    throw Error("Failed to get apisInfo");
                opts.apisInfo = apisInfo;
                opts.target.preferredEditor = pxt.PYTHON_PROJECT_NAME;
                var _a = pxt.py.py2ts(opts), outfiles = _a.outfiles, diagnostics = _a.diagnostics;
                var success = diagnostics.length == 0;
                resp = { outfiles: outfiles, success: success, diagnostics: diagnostics, ast: stsCompRes.ast };
            }
            else {
                resp = pxtc.compile(opts);
            }
            if (resp.outfiles && snippet.file) {
                var dir_1 = snippet.file
                    .replace(/\.ts$/, '')
                    .replace(/\.py$/, '');
            ...
            if (resp.success) {
                if (/^block/.test(snippet.type)) {
                    ...
                    var ts2 = ts2Res.outfiles["main.ts"];
                    if (!ts2) {
...
function dumpheapAsync(c) {
    ensurePkgDir();
    return mainPkg.loadAsync()
        .then(function () { return gdb.dumpheapAsync(c.args[0]); });
}
function dumpmemAsync(c) {
    ensurePkgDir();
    return mainPkg.loadAsync()
        .then(function () { return gdb.dumpMemAsync(c.args); });
}
...
function buildCoreAsync(buildOpts) {
    ...
    var config = nodeutil.readPkgConfig(process.cwd());
    return prepBuildOptionsAsync(buildOpts.mode, false, buildOpts.ignoreTests)
        ...
        var shouldExit = !res.success && buildOpts.mode != BuildOption.GenDocs;
...
function buildAsync(parsed) {
    ...
    return (clean ? cleanAsync() : Promise.resolve())
        ...
        .then(function () {
        parseBuildInfo(parsed);
        return buildCoreAsync({ mode: mode, ignoreTests: ignoreTests });
...
function extractBufferAsync(buf, outDir) {
    ...
    return Promise.resolve()
        ...
        if (json[pxt.CONFIG_NAME]) {
            ...
            var cfg = pxt.Package.parseAndValidConfig(json[pxt.CONFIG_NAME]);
            var files = json;
            json = {
                projects: [{
                        name: cfg.name,
...
function hexdumpAsync(c) {
    ...
    if (/^UF2\n/.test(buf.slice(0, 4).toString("utf8"))) {
        for (var _i = 0, _a = pxtc.UF2.parseFile(buf); _i < _a.length; _i++) {
            var b = _a[_i];
...
function internalCheckDocsAsync(compileSnippets, re, fix, pycheck) {
    ...
    if (fs.existsSync("targetconfig.json")) {
        ...
        if (targeConfig_1.galleries)
            Object.keys(targeConfig_1.galleries)
                .forEach(function (gallery) {
                var url = getGalleryUrl(targeConfig_1.galleries[gallery]);
                todo.push(url);
            });
    }
    ...
    if (nodeutil.fileExistsSync("targetconfig.json")) {
        ...
        if (targetConfig_2 && targetConfig_2.galleries) {
            Object.keys(targetConfig_2.galleries).forEach(function (k) {
                pxt.log("gallery " + k);
                var galleryUrl = getGalleryUrl(targetConfig_2.galleries[k]);
                var gallerymd = nodeutil.resolveMd(docsRoot, galleryUrl);
                var gallery = pxt.gallery.parseGalleryMardown(gallerymd);
                    ...
                    switch (card.cardType) {
                        case "tutorial": {
                            ...
                            if (card.otherActions)
                                card.otherActions.forEach(function (a) { if (a.url)
                                    urls_3.push(a.url); });
                            var _loop_4 = function (url) {
                                var tutorialMd = nodeutil.resolveMd(docsRoot, url);
                                ...
                                if (tutorial.code.indexOf("namespace") !== -1
                                    ...
                                    tutorial.steps
                                        .filter(function (step) { return !!step.contentMd; })
                                        .forEach(function (step, stepIndex) { return getCodeSnippets(gal.name + "-" + stepIndex, step.contentMd)
                                        .forEach(function (snippet, snippetIndex) {
                                        snippet.packages = pkgs;
                                        addSnippet(snippet, "tutorial" + (gal.name + "-" + stepIndex + "-" + snippetIndex), cardIndex);
                                    }); });
                                }
                                else {
                                    addSnippet({
                            ...
                            for (var _i = 0, urls_1 = urls_3; _i < urls_1.length; _i++) {
                                var url = urls_1[_i];
                                _loop_4(url);
                            }
                            break;
                        ...
                        case "example": {
                            ...
                            if (card.otherActions)
                                card.otherActions.forEach(function (a) { if (a.url)
                                    urls_4.push(a.url); });
                            for (var _a = 0, urls_2 = urls_4; _a < urls_2.length; _a++) {
                                var url = urls_2[_a];
                                var exMd = nodeutil.resolveMd(docsRoot, url);
...
function testGithubPackagesAsync(parsed) {
    ...
    var logfile = path.join(pkgsroot, "log.txt");
    var errorfile = path.join(pkgsroot, "error.txt");
    ...
    function reportWarning(er) {
        ...
        var msg = "- [ ] warning: [" + er.repo + "](https://github.com/" + er.repo + ") " + er.title + " / [new issue](https://github.com/" + er.repo + "/issues/new?title=" + encodeURIComponent(er.title) + "&body=" + encodeURIComponent(er.body) + ")";
        fs.appendFileSync(errorfile, msg + "\n");
        warningCount++;
    }
    function reportError(er) {
        reportLog("error: https://github.com/" + er.repo + " " + er.title);
        var msg = "- [ ] error:  [" + er.repo + "](https://github.com/" + er.repo + ") " + er.title + " / [new issue](https://github.com/" + er.repo + "/issues/new?title=" + encodeURIComponent(er.title) + "&body=" + encodeURIComponent(er.body) + ")";
        fs.appendFileSync(errorfile, msg + "\n");
        errorCount++;
    }
    ...
    function buildAsync(pkgpgh, tag) {
        ...
        var buildlog = path.join(pkgdir, "built", "success.txt");
        var buildkey = pxt.appTarget.id + "-" + pxt.appTarget.versions.targetId + "-" + pkgpgh + "-" + tag;
        ...
        return pxt.github.db.loadPackageAsync(pkgpgh, tag)
            ...
            Object.keys(cp.files)
                .forEach(function (fn) { return nodeutil.writeFileSync(path.join(pkgdir, fn), cp.files[fn], "utf8"); });
    ...
    function nextAsync(fullname) {
        ...
        function workAsync() {
            return resolveTagAsync(fullname)
                .then(function (tag) { return buildAsync(fullname, tag); })
                .catch(function (e) {
                if (e.statusCode == 429 && retry++ < 6) {
                    delay *= 2;
                    pxt.log("retrying in " + delay / 1000 + " secs...");
                    return Promise.delay(delay)
                        .then(function () { return workAsync(); });
                }
...
function blockTestsAsync(parsed) {
    ...
    var args = ["start", path.resolve("node_modules/pxt-core/tests/blocks-test/karma.conf.js")];
    if (parsed && parsed.flags["debug"]) {
