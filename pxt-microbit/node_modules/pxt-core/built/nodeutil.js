...
function runNpmAsync() {
    ...
    return runNpmAsyncWithCwd.apply(void 0, __spreadArrays(["."], args));
}
...
exports.cp = cp;
function allFiles(top, maxDepth, allowMissing, includeDirs, ignoredFileMarker) {
    if (maxDepth === void 0) { maxDepth = 8; }
    if (allowMissing === void 0) { allowMissing = false; }
    if (includeDirs === void 0) { includeDirs = false; }
    if (ignoredFileMarker === void 0) { ignoredFileMarker = undefined; }
    var res = [];
...
function resolveMd(root, pathname, md) {
    ...
    var targetMd = md ? md : tryRead(path.join(docs, pathname));
    if (targetMd && !/^\s*#+\s+@extends/m.test(targetMd))
        return targetMd;
    var dirs = __spreadArrays([
...
function getBundledPackagesDocs() {
    ...
    function getPackageDocs(packageDir, folders, resolvedDirs) {
        ...
        var jsonDir = path.join(packageDir, "pxt.json");
        var pxtjson = fs.existsSync(jsonDir) && readJson(jsonDir);
        ...
        if (pxtjson) {
            ...
            if (pxtjson.dependencies) {
                Object.keys(pxtjson.dependencies).forEach(function (dep) {
                    var parts = /^file:(.+)$/i.exec(pxtjson.dependencies[dep]);
                    if (parts) {
