...
function uploadTargetTranslationsAsync(parsed) {
    ...
    var uploadApiStrings = parsed && !!parsed.flags["apis"];
...
function internalUploadTargetTranslationsAsync(uploadApiStrings, uploadDocs) {
    ...
    return crowdinCredentialsAsync()
        ...
        if (crowdinDir == "core") {
            ...
            if (uploadApiStrings)
                p = p.then(function () { return execCrowdinAsync("upload", "built/target-strings.json", crowdinDir); })
                ...
                p = p.then(function () { return pxt.log("translations: skipping api strings upload"); });
            if (uploadDocs)
                p = p.then(function () { return uploadDocsTranslationsAsync("docs", crowdinDir, cred.branch, cred.prj, cred.key); })
                ...
                p = p.then(function () { return pxt.log("translations: skipping docs upload"); });
            return p;
        }
    });
}
